...
if __name__ == '__main__':
    ...
    with open(filename, "r") as csvfile:
        ...
        for index, line in enumerate(lines):
            ...
            raise RuntimeError(
                "Incorrect upload file format. Please include headers.")
        data = '\n'.join(lines[index:])
    ...
    if not prjs:
        prj = session.post(args.url + "ws/projects/",
                           json={"name": project_name, "description": project_name}).json()["value"]
    else:
        prj = prjs[0]
    ...
    for nm, fc in fcs.items():
        if fc["FC"] not in fc2id:
            newfc = session.post(args.url + "ws/fcs/", json={
                                 "name": fc["FC"], "description": "Generated from " + nm}).json()["value"]
    ...
    for fc in fcs.values():
        if (fc["FC"], fc["Fungible"]) not in ffts:
            resp = session.post(args.url + "ws/ffts/",
                                json={"fc": fc["FC"], "fg": fc["Fungible"]})
