...
def initialize_collections():
    ...
    if 'name_1' not in licco_db[line_config_db_name]["fgs"].index_information().keys():
        licco_db[line_config_db_name]["fgs"].create_index([("name", ASCENDING)], unique=True, name="name_1")
    if 'fc_fg_1' not in licco_db[line_config_db_name]["ffts"].index_information().keys():
        licco_db[line_config_db_name]["ffts"].create_index([("fc", ASCENDING), ("fg", ASCENDING)], unique=True, name="fc_fg_1")
    ...
    if 'prj_fc_time_1' not in licco_db[line_config_db_name]["projects_history"].index_information().keys():
        licco_db[line_config_db_name]["projects_history"].create_index([("prj", ASCENDING), ("fft", ASCENDING), ("time", DESCENDING)], name="prj_fft_time_1")
...
def get_all_users():
    ...
    """
    prjs = list(licco_db[line_config_db_name]["projects"].find({ }, {"owner": 1, "editors": 1}))
    ret = set()
    for prj in prjs:
        ret.add(prj["owner"])
...
def get_all_projects(sort_criteria):
    ...
    """
    all_projects = list(licco_db[line_config_db_name]["projects"].find({ }).sort(sort_criteria))
    return all_projects
...
def get_ffts():
    ...
    ffts = list(licco_db[line_config_db_name]["ffts"].aggregate([
        {"$lookup": { "from": "fcs", "localField": "fc", "foreignField": "_id", "as": "fc" }},
        {"$unwind": "$fc"},
        {"$lookup": { "from": "fgs", "localField": "fg", "foreignField": "_id", "as": "fg" }},
        {"$unwind": "$fg"}
    ]))
    ffts_used = set(licco_db[line_config_db_name]["projects_history"].distinct("fft"))
    for fft in ffts:
...
def create_new_fungible_token(name, description):
    ...
    try:
        fgid = licco_db[line_config_db_name]["fgs"].insert_one({ "name": name, "description": description }).inserted_id
...
def copy_ffts_from_project(srcprjid, destprjid, fftid, attrnames, userid):
    ...
    """
    srcprj = licco_db[line_config_db_name]["projects"].find_one({"_id": ObjectId(srcprjid)})
    if not srcprj:
        return False, f"Cannot find project for {srcprj}", None
    destprj = licco_db[line_config_db_name]["projects"].find_one({"_id": ObjectId(destprjid)})
    if not destprj:
    ...
    current_attrs = get_project_attributes(licco_db[line_config_db_name], ObjectId(destprjid))
    fftattrs = current_attrs.get(fftid, {})
    other_attrs = get_project_attributes(licco_db[line_config_db_name], ObjectId(srcprjid))
    ...
    for attrname, cnvattrval in oattrs.items():
        ...
        if fftattrs.get(attrname) != cnvattrval:
            logger.debug("Updating %s in prj %s to %s", attrname, destprjid, cnvattrval)
            all_inserts.append({
...
def submit_project_for_approval(prjid, userid, approver):
    ...
    licco_db[line_config_db_name]["projects"].update_one({"_id": prj["_id"]}, {"$set": {"status": "submitted", "submitter": userid, "approver": approver, "submitted_time": datetime.datetime.utcnow()}})
    return True, "", prj
...
def approve_project(prjid, userid):
    ...
    licco_db[line_config_db_name]["projects"].update_many({"status": "approved"}, {"$set": {"status": "archived" }})
    licco_db[line_config_db_name]["projects"].update_one({"_id": prj["_id"]}, {"$set": {"status": "approved", "approver": userid, "approved_time": datetime.datetime.utcnow()}})
    return True, "", prj
...
def reject_project(prjid, userid, reason):
    ...
    if prj["status"] != "submitted":
        return False, f"Project {prjid} is not in submitted status", None
    licco_db[line_config_db_name]["projects"].update_one({"_id": prj["_id"]}, {"$set": {"status": "development", "approver": userid, "approved_time": datetime.datetime.utcnow(), "notes": [reason] + prj.get("notes", [])}})
    return True, "", prj
...
def create_empty_project(name, description, logged_in_user):
    ...
    """
    prjid = licco_db[line_config_db_name]["projects"].insert_one({"name": name, "description": description, "owner": logged_in_user, "editors" : [], "status": "development", "creation_time": datetime.datetime.utcnow() }).inserted_id
...
def update_project_details(prjid, prjdetails):
    ...
    """
    licco_db[line_config_db_name]["projects"].update_one({"_id": ObjectId(prjid)}, {"$set": {"name": prjdetails["name"], "description": prjdetails["description"] }})
    return True
