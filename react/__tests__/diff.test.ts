...
const createMockDeviceDetails = (fc: string, fg: string) => {
    let d: ProjectDeviceDetails = {
        _id: '',
        fc: fc, fg_desc: fg, area: '', beamline: [], state: DeviceState.Conceptual.name, tc_part_no: "", comments: "", stand: '', discussion: [],
        device_id: 0, device_type: 2, created: new Date(), project_id: ''
    }
    return d;
}
...
test('diff algorithm grouping', () => {
    ...
    { // check if found the right new device
        ...
        expect(newDevice.fg_desc).toBe("NEW_DEVICE");
    }
    ...
    { // check if we found the right missing device
        ...
        expect(missingDevice.fg_desc).toBe("MISSING_DEVICE");
    ...
    { // check if we found the right updated device
        ...
        expect(updatedA.fg_desc).toBe("UPDATED_DEVICE");
        expect(updatedA.ray_trace).toBe(0);

        let updatedB = updatedDevices.b;
        expect(updatedB.fc).toBe("A");
        expect(updatedB.fg_desc).toBe("UPDATED_DEVICE");
    ...
    { // check if we found the right identical device
        ...
        expect(identicalDevice.fg_desc).toBe("IDENTICAL_DEVICE");
